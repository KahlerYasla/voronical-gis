--This query returns the optimal location based on voroni polygons if a new market will be opened
...
clipped_voronoi AS (
    ...
    FROM voronoi_dump v, boundary b
),
-- Find the largest Voronoi polygon by area
largest_voronoi AS (
    SELECT
        geom,
...
-- Get the centroid of the largest Voronoi polygon
SELECT
    ST_Centroid(geom) AS recommended_location
FROM largest_voronoi;
